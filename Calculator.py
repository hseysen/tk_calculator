...
class Calculator(tk.LabelFrame):
    ...
    def create_widgets(self):
        ...
        self.entrybox.configure(state=tk.DISABLED, disabledbackground=ENTRYBG, disabledforeground=ENTRYFG)
    ...
    def create_button(self, displaytext, i, j, cmd):
        b = tk.Button(self, font=TEXTFONT, text=displaytext, command=cmd)
        b.grid(row=i, column=j, sticky=STICKYTEXT)
        b.configure(bg=BUTTONBG, fg=BUTTONFG, activebackground=BUTTONACTIVEBG)
        return b
    ...
    def calculate(self):
        ...
        calculation = calculation.replace(SQRTSYMBOL, "sqrt").replace(MULTIPLYSYMBOL, "*")
        while "**" in calculation:
            calculation = calculation.replace("**", "*")
        calculation = calculation.replace("^", "**").replace(DIVISIONSYMBOL, "/").rstrip("=")
